...
public class ElectricalSensorGui extends GuiContainerEln {
    ...
    public ElectricalSensorGui(Player player, Container inventory, ElectricalSensorRender render) {
        super(new ElectricalSensorContainer(player, inventory, render.descriptor), player.getInventory(), Component.literal("Electrical Sensor"));
        this.render = render;
    }
        ...
        if (!render.descriptor.voltageOnly) {
            switch (render.dirType) {
                case ElectricalSensorElement.dirNone:
                    dirType.setMessage(Component.literal("\u00a72\u25CF\u00a77 <=> \u00a71\u25CF"));
                    break;
