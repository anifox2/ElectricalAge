...
class WaterTurbineElement(
    ...
    class WaterTurbineSlowProcess(val turbine: WaterTurbineElement) : IProcess, INBTTReady {
        ...
        override fun process(time: Double) {
            ...
            if (refreshTimeout < 0) {
                ...
                val waterFactor = getWaterFactor()
                if (waterFactor < 0) {
                    filter.setValue((filter.get() * (1 - 0.5f * time)).toFloat())
                } else {
                    filter.target = (waterFactor * d.nominalPower).toFloat()
        ...
        private fun getWaterFactor(): Double {
            return 1.0
        }
