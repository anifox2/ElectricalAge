...
class ModbusRtuDescriptor(name: String, var obj: Obj3D?) : SixNodeDescriptor(name, ModbusRtuElement::class.java, ModbusRtuRender::class.java) {
    ...
    fun draw(doorAlpha: Float, power: Boolean, activity: Boolean, error: Boolean) {
        if (main != null) main!!.draw()
        if (door != null) {
            GL11.glPushMatrix()
            GL11.glRotatef(doorAlpha, 0f, 0f, 1f)
            door!!.draw()
            GL11.glPopMatrix()
        }
        if (power) UtilsClient.drawLight(led_power) else if (led_power != null) led_power!!.draw()
        if (activity) UtilsClient.drawLight(led_activity) else if (led_activity != null) led_activity!!.draw()
        if (error) UtilsClient.drawLight(led_error) else if (led_error != null) led_error!!.draw()
        if (display != null) display!!.draw()
    }
}
